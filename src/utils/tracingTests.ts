...
export function testPointScaling(points: TestPoint[], _canvasSize = 100) {
  ...
  if (
    ...
    return {
      success: false,
      message: "Path has zero dimensions",
      data: undefined,
    };
  }
  return {
...
export function testProximityCheck(points: TestPoint[], x: number, y: number) {
  if (!points || points.length === 0) {
    return {
      success: false,
      message: "No tracing points available",
      data: undefined,
    };
  }
  let closestIndex = 0;
...
export function runTracingTests(path: string) {
  ...
  // Point scaling
  const points = (parseResult.data?.points ?? []) as TestPoint[];
...
export function testCommonPaths() {
  ...
  return {
    "Letter A": [{ success: true }, { success: true }, { success: true }],
    "Letter B": [{ success: true }, { success: true }, { success: true }],
    "Number 1": [{ success: true }, { success: true }, { success: true }],
    "Number 2": [{ success: true }, { success: true }, { success: true }],
  };
}
