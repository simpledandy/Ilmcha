...
// Helper: split pathData into segments and assign pointers
function splitPathToSegments(pathData: string): { path: string, pointer: [number, number], direction?: 'left' | 'right' | 'down' | 'up' }[] {
  // Split on 'M' (move to), keep the first 'M' in each segment
  const parts = pathData.match(/M[^M]+/g);
  if (!parts) return [];
  return parts.map(seg => {
    // Find the first coordinate after 'M'
    const match = seg.match(/M\s*([\d.\-]+),([\d.\-]+)/);
    let pointer: [number, number] = [200, 200];
    if (match) {
      pointer = [parseFloat(match[1]), parseFloat(match[2])];
    }
    // TODO: Optionally infer direction
    const direction: 'left' | 'right' | 'down' | 'up' | undefined = undefined;
    return { path: seg.trim(), pointer, direction };
  });
}

// New: get tracing segments by category
export const getTracingSegmentsByCategory = (category: 'number' | 'letter', language: 'en' | 'uz' = 'en') => {
  if (category === 'number') {
    return Object.entries(numberPaths).map(([value, pathData]) => ({
      id: `number-${value}`,
      segments: splitPathToSegments(pathData),
      startPoint: { x: 0, y: 0 },
      endPoint: { x: 0, y: 0 },
      difficulty: 'medium' as const,
      category: 'number' as const,
