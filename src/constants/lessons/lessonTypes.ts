import { LessonContent, LessonTemplateParams } from '../../types/common';
...
export interface TracingExercise {
   ...
   */
  pathData: string;
  segments?: { path: string; pointer: [number, number]; direction?: 'left' | 'right' | 'down' | 'up' }[];
...
export function createLesson<T extends keyof typeof lessonTemplates>(
  ...
  return (lessonTemplates[template] as (...args: Parameters<typeof lessonTemplates[T]>) => Lesson)(...params);
}
